...
@app.route("/hospital_specialisation",methods=['GET'])
def get_doctor_specilisation_t() :
    args = request.get_json()
    # encoded = requests.cookies.get('PatientAuth')
    # decoded = jwt.decode(encoded, key, algorithms=["RS256"])
    # jwt = decoded['nhid']
    ...
    if specialisation != '' : 
    # remove the doctor from the list
        query = f'SELECT hospital_id  from specialization WHERE specialisation = %s'
        cur.execute(query,(specialisation))
        record = cur.fetchall()
        return json.dumps(record)
    else :
        query = f'SELECT hospital_id  from specialization'
        cur.execute(query)
        record = cur.fetchall()
        return json.dumps(record)
...
def get_doctor_specilisation_zz() :
    args = request.get_json()
    encoded = requests.cookies.get('PatientAuth')
    decoded = jwt.decode(encoded, key, algorithms=["RS256"])
    # jwt = decoded['nhid']
    ...
    # remove the doctor from the list
    if specialisation != '' :
        query = f'SELECT doc_id  from specialization WHERE specialisation = %s AND hospital_id = %s'
        cur.execute(query,(specialisation,hospital_id))
        record = cur.fetchall()
        return record
    else :
        query = f'SELECT doc_id  from specialization WHERE hospital_id = %s'
        cur.execute(query,(specialisation,hospital_id))
        record = cur.fetchall()
        return record
